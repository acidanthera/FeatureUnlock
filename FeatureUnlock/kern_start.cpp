...
static_assert(sizeof(kMacModelSidecarPart1Original) == sizeof(kMacModelSidecarPart1Patched), "patch size invalid");
static_assert(sizeof(kMacModelSidecarPart2Original) == sizeof(kMacModelSidecarPart2Patched), "patch size invalid");
...
static boolean_t patched_cs_validate_range(vnode_t vp, memory_object_t pager, memory_object_offset_t offset, const void *data, vm_size_t size, unsigned *result) {
    ...
    if (res && vn_getpath(vp, path, &pathlen) == 0 && UserPatcher::matchSharedCachePath(path)) {
        searchAndPatch(data, size, path, kMacModelSidecarPart1Original, kMacModelSidecarPart1Patched);
        searchAndPatch(data, size, path, kMacModelSidecarPart2Original, kMacModelSidecarPart2Patched);
...
static void patched_cs_validate_page(vnode_t vp, memory_object_t pager, memory_object_offset_t page_offset, const void *data, int *validated_p, int *tainted_p, int *nx_p) {
	...
	if (vn_getpath(vp, path, &pathlen) == 0 && UserPatcher::matchSharedCachePath(path)) {
        searchAndPatch(data, PAGE_SIZE, path, kMacModelSidecarPart1Original, kMacModelSidecarPart1Patched);
        searchAndPatch(data, PAGE_SIZE, path, kMacModelSidecarPart2Original, kMacModelSidecarPart2Patched);
